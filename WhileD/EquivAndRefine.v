...
Proof.
  intros.
  unfold test_true; unfold_RELS_tac; intros s1 s2.
  intros [ [i [? ?] ] ?].
  apply H.(nrm_erefine) in H0.
  sets_unfold in H0.
  destruct H0; [| tauto].
...
Proof.
  intros.
  unfold test_false; unfold_RELS_tac; intros s1 s2.
  intros [? ?].
  apply H.(nrm_erefine) in H0.
  sets_unfold in H0.
  destruct H0; [| tauto].
...
Lemma iter_nrm_lt_mono_aux:
  forall (e1 e2: expr) (c1 c2: com) n,
    e1 <<= e2 ->
    c1 <<= c2 ->
    iter_nrm_lt_n ⟦ e1 ⟧ ⟦ c1 ⟧ n ⊆
    iter_nrm_lt_n ⟦ e2 ⟧ ⟦ c2 ⟧ n ∪
    (iter_err_lt_n ⟦ e2 ⟧ ⟦ c2 ⟧ n × state).
Proof.
  intros.
  induction n; simpl.
...
Lemma iter_err_lt_mono_aux:
  forall (e1 e2: expr) (c1 c2: com) n,
    e1 <<= e2 ->
    c1 <<= c2 ->
    iter_err_lt_n ⟦ e1 ⟧ ⟦ c1 ⟧ n ⊆
    iter_err_lt_n ⟦ e2 ⟧ ⟦ c2 ⟧ n.
Proof.
  intros.
  induction n; simpl.
  + apply Sets_empty_included.
...
Proof.
  intros.
  unfold Sets.complement; sets_unfold.
  split.
  + apply not_ex_all_not.
  + apply all_not_not_ex.
Qed.

Definition iter_not_error (D1: EDenote) (D2: CDenote): state -> Prop :=
  Sets.complement (⋃ (iter_err_lt_n D1 D2)).

Lemma iter_err_fact:
  forall (D1: EDenote) (D2: CDenote),
    test_true D1 ∘ D2.(nrm) ∘ ⋃ (iter_err_lt_n D1 D2) ⊆
    ⋃ (iter_err_lt_n D1 D2).
Proof.
  intros.
...
Proof.
  intros.
  unfold Sets.complement; unfold_RELS_tac; intros a.
  intros [ [b [? ?] ] ?].
  exists b.
  split; [tauto |].
  split; [tauto |].
...
Lemma iter_not_error_fact1:
  forall (D1: EDenote) (D2: CDenote),
    D1.(err) ∩ iter_not_error D1 D2 == ∅.
Proof.
  intros.
  unfold iter_not_error.
  ...
  simpl.
  unfold Sets.complement; sets_unfold; intros s.
  tauto.
Qed.

Lemma iter_not_error_fact2:
  forall (D1: EDenote) (D2: CDenote),
    (test_true D1 ∘ D2.(err)) ∩ iter_not_error D1 D2 == ∅.
Proof.
  intros.
  unfold iter_not_error.
  rewrite Sets_complement_indexed_union.
...
Instance Sets_complement_mono: forall {A: Type},
  Proper (Sets.included --> Sets.included) (@Sets.complement A).
Proof.
  intros.
  unfold Proper, respectful, Basics.flip.
  unfold Sets.complement; sets_unfold; intros.
  specialize (H a); tauto.
Qed.

Lemma iter_inf_lt_mono_aux:
  forall (e1 e2: expr) (c1 c2: com) X,
    e1 <<= e2 ->
    c1 <<= c2 ->
    is_inf ⟦ e1 ⟧ ⟦ c1 ⟧ X ->
    is_inf ⟦ e2 ⟧ ⟦ c2 ⟧ (X ∩ iter_not_error ⟦ e2 ⟧ ⟦ c2 ⟧).
Proof.
  unfold is_inf.
  intros.
  rewrite H1 at 1.
  ...
  rewrite iter_not_error_fact1.
  rewrite iter_not_error_fact2.
  rewrite ! Sets_union_empty.
  unfold iter_not_error at 1.
  rewrite <- iter_err_fact at 1.
  rewrite ! Rels_concat_excluding_r.
  fold (iter_not_error ⟦ e2 ⟧ ⟦ c2 ⟧).
  sets_unfold; intros s; tauto.
Qed.
...
Proof.
  ...
  + apply Sets_indexed_union_included; intros n.
    ...
    apply iter_nrm_lt_mono_aux; tauto.
  + apply Sets_indexed_union_included; intros n.
    rewrite <- (Sets_included_indexed_union _ _ n).
    apply iter_err_lt_mono_aux; tauto.
  + apply Sets_general_union_included.
    intros X ?.
    pose proof iter_inf_lt_mono_aux _ _ _ _ _ H H0 H1.
    rewrite <- (Sets_included_general_union _ (X ∩ iter_not_error ⟦ e2 ⟧ ⟦ c2 ⟧))
      by (exact H2).
    unfold iter_not_error.
    unfold Sets.complement; sets_unfold; intros s; tauto.
Qed.
